...
/**
 * Base class that represents a row from the 'short_link' table.
 *
 * 
 *
...
abstract class BaseShortLink extends BaseObject  implements Persistent {
	...
	{
		if(!isset($this->oldColumnsValues[ShortLinkPeer::ID]))
			$this->oldColumnsValues[ShortLinkPeer::ID] = $this->id;
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = ShortLinkPeer::ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ShortLinkPeer::INT_ID]))
			$this->oldColumnsValues[ShortLinkPeer::INT_ID] = $this->int_id;
		...
		if ($this->int_id !== $v) {
			$this->int_id = $v;
			$this->modifiedColumns[] = ShortLinkPeer::INT_ID;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = ShortLinkPeer::CREATED_AT;
			}
	...
	public function setUpdatedAt($v)
	{
		...
		if ( $this->updated_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = ShortLinkPeer::UPDATED_AT;
			}
		} // if either are not null
	...
	{
		if(!isset($this->oldColumnsValues[ShortLinkPeer::EXPIRES_AT]))
			$this->oldColumnsValues[ShortLinkPeer::EXPIRES_AT] = $this->expires_at;
		...
		if ( $this->expires_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = ShortLinkPeer::EXPIRES_AT;
			}
		} // if either are not null
	...
	{
		if(!isset($this->oldColumnsValues[ShortLinkPeer::PARTNER_ID]))
			$this->oldColumnsValues[ShortLinkPeer::PARTNER_ID] = $this->partner_id;
		...
		if ($this->partner_id !== $v) {
			$this->partner_id = $v;
			$this->modifiedColumns[] = ShortLinkPeer::PARTNER_ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ShortLinkPeer::NAME]))
			$this->oldColumnsValues[ShortLinkPeer::NAME] = $this->name;
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = ShortLinkPeer::NAME;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ShortLinkPeer::SYSTEM_NAME]))
			$this->oldColumnsValues[ShortLinkPeer::SYSTEM_NAME] = $this->system_name;
		...
		if ($this->system_name !== $v) {
			$this->system_name = $v;
			$this->modifiedColumns[] = ShortLinkPeer::SYSTEM_NAME;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ShortLinkPeer::FULL_URL]))
			$this->oldColumnsValues[ShortLinkPeer::FULL_URL] = $this->full_url;
		...
		if ($this->full_url !== $v) {
			$this->full_url = $v;
			$this->modifiedColumns[] = ShortLinkPeer::FULL_URL;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ShortLinkPeer::STATUS]))
			$this->oldColumnsValues[ShortLinkPeer::STATUS] = $this->status;
		...
		if ($this->status !== $v) {
			$this->status = $v;
			$this->modifiedColumns[] = ShortLinkPeer::STATUS;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 11; // 11 = ShortLinkPeer::NUM_COLUMNS - ShortLinkPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				ShortLinkPeer::doDelete($this, $con);
				$this->postDelete($con);
	...
	public function save(PropelPDO $con = null, $skipReload = false)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				ShortLinkPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedRows += ShortLinkPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = ShortLinkPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = ShortLinkPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = ShortLinkPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ShortLinkPeer::ID)) $criteria->add(ShortLinkPeer::ID, $this->id);
		if ($this->isColumnModified(ShortLinkPeer::INT_ID)) $criteria->add(ShortLinkPeer::INT_ID, $this->int_id);
		if ($this->isColumnModified(ShortLinkPeer::CREATED_AT)) $criteria->add(ShortLinkPeer::CREATED_AT, $this->created_at);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(ShortLinkPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of ShortLink (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     ShortLink Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @var     ShortLink Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      ShortLink $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(ShortLink $copiedFrom)
	{
	 ...
	 *
	 * @return     ShortLinkPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new ShortLinkPeer();
		}
		return self::$peer;
	}
