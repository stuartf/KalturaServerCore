 ...
 */
class VisoService extends KalturaBaseService
{
	...
	public function getFeedAction($distributionProfileId, $hash)
	{
		...
		if (!$profile || !$profile instanceof VisoDistributionProfile)
		...
		if ($profile->getUniqueHashForFeedUrl() != $hash)
			throw new KalturaAPIException(VisoDistributionErrors::INVALID_FEED_URL);
		...
		$feed = new VisoFeed('viso_template.xml');
		...
		foreach($entries as $entry)
		{
			...
			assetPeer::resetInstanceCriteriaFilter(); // there is some default filter that is adding (type == 2)
