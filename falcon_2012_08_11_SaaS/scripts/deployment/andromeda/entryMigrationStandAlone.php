...
if($loopIndex == 'single')
{
	...
	migrateEntries::migrateSingleEntry($entry, $partner);
}
else
{
	AndromedaMigration::doEntries($partner, $work_from_date, $loopSize, $loopIndex);
}
exit(0);
...
class AndromedaMigration
{
	...
	public static function doEntries($partner, $work_from_date, $loopSize, $loopIndex)
	{
		$c = new Criteria();
		$c->addAnd(entryPeer::PARTNER_ID, $partner->getId());
		if($work_from_date)
		{
			$c->addAnd(entryPeer::UPDATED_AT, $work_from_date, Criteria::GREATER_EQUAL);
		}	
		...
		if($res == 0 || $res == 2)
		{
			$failedIds[$loopIndex] = migrateEntries::getFailedIds();
		}
		else
		{
		}
