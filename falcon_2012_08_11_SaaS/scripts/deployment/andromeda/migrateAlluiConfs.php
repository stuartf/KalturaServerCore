...
while(1)
{
	...
	foreach($partners as $partner)
	{
		...
		AndromedaMigration::douiconfs($partner);
	}
}
exit();
exit(0);
...
class AndromedaMigration
{
	...
	public static function initScript($partner)
	{
		...
		$logFile_dir = 'logs/uiconf_mig/'.$partner_prefix.'/';
		$partner_log_file = $logFile_dir.$partner->getId().'_uiconfs.log';
	...
	public static function douiconfs($partner)
	{
		self::$offset = 0;
		while(1)
		{
			...
			$res = migrateUiconfs::migrateUiconfList($confs, $partner);
			if($res == 0 || $res == 2)
			{
				$failedIds[self::$offset] = migrateUiconfs::getFailedIds();
			}
			else
			{
			}
