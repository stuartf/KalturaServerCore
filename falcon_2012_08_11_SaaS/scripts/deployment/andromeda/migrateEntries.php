...
class migrateEntries extends AndromedaMigration
{
	...
	public static function migrateSingleEntry(entry $entry, $partner = null)
	{
		...
		{
			$isImage = ($entry->getType() == entryType::MEDIA_CLIP && $entry->getMediaType() == entry::ENTRY_MEDIA_TYPE_IMAGE);
			$isNotClip = ($entry->getType() != entryType::MEDIA_CLIP);
	...
	public static function migrateEntryList($arrEntries, $partner = null)
	{
		...
		{
			if($entry->getStatus() == entryStatus::DELETED) continue; // ignore deleted entries
