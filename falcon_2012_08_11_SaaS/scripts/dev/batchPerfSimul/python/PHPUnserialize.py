...
class PHPUnserialize(object):
    ...
    def _unserialize(self, data, offset=0):
        ...
        # when serializing and the original type of the array is lost
            ...
            objName = None
            if dtype == 'o':
                (chars, nameLen) = self.read_until(data, dataoffset, ':')
                dataoffset += chars + 2
                (chars, objName) = self.read_chars(data, dataoffset, int(nameLen) + 2)
                dataoffset += chars
            ...
            if objName != None:
                ...
                for key, value in readdata.items():
                    memberName = key[(key.rfind('\0') + 1):]
                    newreaddata[memberName] = value
