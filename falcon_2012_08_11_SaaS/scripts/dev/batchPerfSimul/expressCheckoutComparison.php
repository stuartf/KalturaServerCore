...
$SIMULATIONS = array();
for ($expressMachineCount = 0; $expressMachineCount < 16; $expressMachineCount += 2)
{
	$curTopology = array(
...
foreach ($SCHEDULERS as $scheduler)
{
	...
	foreach ($vars as $var)
	{
		...
		for ($expressMachineCount = 0; $expressMachineCount < 16; $expressMachineCount += 2)
		{
			$simulation = $simulations["$scheduler/{$expressMachineCount}_Limited"];
