...
defaults('ADMIN_PASSWORD','pacapaca');  	// Admin Password - CHANGE THIS TO ENABLE!!!
...
<?php
	...
	$mem_util=round(($mem_used/$mem_size)*100,2);
	$filecache_hitratio=round(($cache['num_hits']*100/($cache['num_hits']+$cache['num_misses'])),3);
	$usercache_hitratio=round(($cache_user['num_hits']*100/($cache_user['num_hits']+$cache_user['num_misses'])),3);
	$i=0;
	echo <<< EOB
...
FleCache.Hits={$cache['num_hits']}
FileCache.Misses={$cache['num_misses']}
FileCache.HitRatio={$filecache_hitratio}
UserCache.Hits={$cache_user['num_hits']}
UserCache.Misses={$cache_user['num_misses']}
UserCache.HitRatio={$usercache_hitratio}
