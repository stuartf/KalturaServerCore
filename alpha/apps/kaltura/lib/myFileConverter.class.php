...
//		$conversion_string .= " -lavfopts i_certify_that_my_video_stream_does_not_use_b_frames " ;
	...
	{
		KalturaLog::log("convertImage - source before rotation [$source_file] target is [$target_file]");
		// make a new copy of source so it will not be overwritten
		...
		$new_source_file = "/tmp/cache_".pathinfo($source_file, PATHINFO_FILENAME)."_orientation_temp.".microtime(true).".".pathinfo($source_file, PATHINFO_EXTENSION);
		copy($source_file, $new_source_file);
		// rotate first to avoid mixed-up width/height so all other actions will be done on the correct dimensions
		$source_file = self::rotate($source_file, $new_source_file);
		KalturaLog::log("convertImage - source after rotation [$source_file] target was [$target_file]");
		list($sourcewidth, $sourceheight, $type, $attr, $srcIm) = self::createImageByFile($source_file);
		if (!$srcIm)
		{
	...
	private static function rotate($data_file, $target_file)
	{
		$exif_data = @exif_read_data($data_file);
		...
		$cmd = "convert $options $target_file $target_file.rot";
		$retValue = null;
		...
		// try to rename the original to backup
		if(!rename($target_file, "$target_file.bk"))
			return $target_file;
		
		// try to rename the rotated to the target
		if(rename("$target_file.rot", $target_file))	
			unlink("$target_file.bk"); // delete backup
		else
			rename("$target_file.bk", $target_file); // return to backup
