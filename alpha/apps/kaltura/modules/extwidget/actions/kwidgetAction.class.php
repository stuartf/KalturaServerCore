	...
	public function execute()
	{
		...
		if ($uiConf)
		{
			...
			else
			{
				...
				else if ( false && $partner_type == 101 || $widget_type == 11118 || $partner_id == 19650 || $partner_id == 2217 || ( $gallery_widget && ($partner_id == 593 || $partner_id == 9197 || $partner_id == 6095 ) ) )
				{
					$ks = "";
					$result = kSessionUtils::createKSessionNoValidations ( $partner_id , 0 , $ks , 86400 , false , "" , "view:*" );
					$ks_flashvars = "&$partnerIdStr&uid=0&ts=".microtime(true);
					...
					// we don't want this for WordPress versions and athletix nation
					if ($partner_id != 2217 && $partner_type != 101 && ( !file_exists($cached_swf) || filemtime($cached_swf) < $confFile_mtime))
					{
						kFile::fullMkdir($cached_swf);
						
						// hack for kdp v1.2.3 - the kdp expect only the layout to be patched and not the whole getwidget call
						if ($partner_id == 6095 || $widget_type == 11118)
						{
							$result = $uiConf->getConfFile();
						}
						else
						{
							$kaltura_services = new getwidgetAction();
							$kaltura_services->setInputParams(
							array (
									"format" => kalturaWebserviceRenderer::RESPONSE_TYPE_XML, 
									"detailed" => 1, 
									"partner_id" => $partner_id, 
									"widget_id" => $widget_id,
									"uiconf_id" => $widget_type
									)
									);
