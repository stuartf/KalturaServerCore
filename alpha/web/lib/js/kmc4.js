...
kmc.functions = {
	...
	openKcw : function(ks, conversion_profile) {
		...
		modal = kalturaInitModalBox ( null , {
			width: 700,
			height: 360
		} );
...
kmc.utils = {
	...
	secureIframe : function(action, fields) {
		...
		var modal_width = 370;
		var modal_height = (action  == "name") ? 200 : 180 ;
...
kmc.mediator =  {
	...
	loadModule : function(module,subtab) {
		...
		var module_url = {
			data : eval("kmc.modules." + module + ".swf_url")
			};
		//	alert("module_url="+module_url.data);
		var attributes = kmc.utils.mergeJson(kmc.modules.shared.attributes,module_url);
		var flashvars = kmc.utils.mergeJson(kmc.modules.shared.flashvars,eval("kmc.modules." + module + ".flashvars"),{
			subNavTab : subtab
		});
		flashvars = {
...
kmc.preview_embed = {
	...
	doPreviewEmbed : function(id, name, description, previewOnly, is_playlist, uiconf_id, live_bitrates, has_mobile_flavors, html5_compatible) {
		...
		modal = kalturaInitModalBox ( null , {
			...
			height: parseInt(uiconf_details.height) + 200
			} );
	...
	doFlavorPreview : function(entry_id, entry_name, flavor_details) {
		...
		modal = kalturaInitModalBox ( null , {
			...
			height: parseInt(kmc.vars.default_kdp.height) + 10
			} );
		$("#mbContent").addClass("new");
	...
	jw : {
		...
		buildJWEmbed : function (entry_id, name, description, is_playlist, uiconf_id) {
			...
			/* for AdSolution */
			var jw_ads = {
				channel : $("#adSolution_channel").val()
				};
