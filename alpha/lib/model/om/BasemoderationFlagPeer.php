...
/**
 * Base static class for performing query and update operations on the 'moderation_flag' table.
 *
 * 
 *
 * @package Core
...
abstract class BasemoderationFlagPeer {
	...
	/**
	 * An identiy map to hold any loaded instances of moderationFlag objects.
	 * This must be public so that other peer classes can access this when hydrating from JOIN
	 * queries.
	 * @var        array moderationFlag[]
	 */
	 ...
	 * @param      string $column The column name for current table. (i.e. moderationFlagPeer::COLUMN_NAME).
	 * @return     string
	 */
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		$criteria->setPrimaryTableName(moderationFlagPeer::TABLE_NAME);
	...
	public static function updateInstancePool(&$queryResult)
	{
		...
		{
			$objFromPool = moderationFlagPeer::getInstanceFromPool($curObject->getPrimaryKey());
			if ($objFromPool === null)
			{
				moderationFlagPeer::addInstanceToPool($curObject);
			}
			else
			{
	...
	public static function addInstancesToPool($queryResult)
	{
		...
		{
			moderationFlagPeer::addInstanceToPool($curResult);
		}
	}
	...
	{
		$criteria_filter = moderationFlagPeer::getCriteriaFilter();
	 ...
	 *
	 * @param      moderationFlag $value A moderationFlag object.
	 * @param      string $key (optional) key to use for instance map (for performance boost if key was already calculated externally).
	 */
	public static function addInstanceToPool(moderationFlag $obj, $key = null)
	{
	 ...
	 *
	 * @param      mixed $value A moderationFlag object or a primary key value.
	 */
	public static function removeInstanceFromPool($value)
	{
		if (Propel::isInstancePoolingEnabled() && $value !== null) {
			if (is_object($value) && $value instanceof moderationFlag) {
				$key = (string) $value->getId();
			} elseif (is_scalar($value)) {
				// assume we've been passed a primary key
				$key = (string) $value;
			} else {
				$e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or moderationFlag object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value,true)));
				throw $e;
			}
	 ...
	 * @return     moderationFlag Found object or NULL if 1) no instance exists for specified key or 2) instance pooling has been disabled.
	...
	/**
	 * Method to invalidate the instance pool of all tables related to moderation_flag
	...
	public static function populateObjects(PDOStatement $stmt)
	{
		...
		$cls = moderationFlagPeer::getOMClass(false);
		// populate the object(s)
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$key = moderationFlagPeer::getPrimaryKeyHashFromRow($row, 0);
			if (null !== ($obj = moderationFlagPeer::getInstanceFromPool($key))) {
	 ...
	 * @param      PropelPDO $con
	 * @param      String    $join_behavior the type of joins to use, defaults to Criteria::LEFT_JOIN
	 * @return     array Array of moderationFlag objects.
	 * @throws     PropelException Any exceptions caught during processing will be
	 *		 rethrown wrapped into a PropelException.
	 */
	public static function doSelectJoinkuserRelatedByKuserId(Criteria $criteria, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$key1 = moderationFlagPeer::getPrimaryKeyHashFromRow($row, 0);
			if (null !== ($obj1 = moderationFlagPeer::getInstanceFromPool($key1))) {
				// We no longer rehydrate the object, since this can cause data loss.
				// See http://propel.phpdb.org/trac/ticket/509
	...
	/**
	 * Selects a collection of moderationFlag objects pre-filled with their entry objects.
	 * @param      Criteria  $criteria
	 * @param      PropelPDO $con
	 * @param      String    $join_behavior the type of joins to use, defaults to Criteria::LEFT_JOIN
	 * @return     array Array of moderationFlag objects.
	...
	public static function doSelectJoinentry(Criteria $criteria, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			if ($key2 !== null) {
				...
				// Add the $obj1 (moderationFlag) to $obj2 (entry)
	...
	/**
	 * Selects a collection of moderationFlag objects pre-filled with all related objects.
	 *
	 * @param      Criteria  $criteria
	 * @param      PropelPDO $con
	 * @param      String    $join_behavior the type of joins to use, defaults to Criteria::LEFT_JOIN
	...
	public static function doSelectJoinAll(Criteria $criteria, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
          ...
          $cls = substr('.'.$omClass, strrpos('.'.$omClass, '.') + 1);
				...
				// Add the $obj1 (moderationFlag) to the collection in $obj3 (entry)
	...
	public static function doSelectJoinAllExceptkuserRelatedByKuserId(Criteria $criteria, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
            ...
            $cls = substr('.'.$omClass, strrpos('.'.$omClass, '.') + 1);
				...
				// Add the $obj1 (moderationFlag) to the collection in $obj2 (entry)
				$obj2->addmoderationFlag($obj1);
	...
	/**
	 * Selects a collection of moderationFlag objects pre-filled with all related objects except entry.
	 *
	 * @param      Criteria  $criteria
	 * @param      PropelPDO $con
	 * @param      String    $join_behavior the type of joins to use, defaults to Criteria::LEFT_JOIN
	...
	{
	  $dbMap = Propel::getDatabaseMap(BasemoderationFlagPeer::DATABASE_NAME);
	  if (!$dbMap->hasTable(BasemoderationFlagPeer::TABLE_NAME))
	  {
	    $dbMap->addTableObject(new moderationFlagTableMap());
	  }
	}
	...
	{
		return $withPrefix ? moderationFlagPeer::CLASS_DEFAULT : moderationFlagPeer::OM_CLASS;
	}

	/**
	 * Method perform an INSERT on the database, given a moderationFlag or Criteria object.
	 *
	 * @param      mixed $values Criteria or moderationFlag object containing data that is used to create the INSERT statement.
	...
	public static function doInsert($values, PropelPDO $con = null)
	{
		...
		if ($values instanceof Criteria) {
			...
			$criteria = $values->buildCriteria(); // build Criteria from moderationFlag object
		}

		if ($criteria->containsKey(moderationFlagPeer::ID) && $criteria->keyContainsValue(moderationFlagPeer::ID) ) {
			throw new PropelException('Cannot insert a value for auto-increment primary key ('.moderationFlagPeer::ID.')');
		}
	...
	/**
	 * Method perform an UPDATE on the database, given a moderationFlag or Criteria object.
	 *
	 * @param      mixed $values Criteria or moderationFlag object containing data that is used to create the UPDATE statement.
	...
	public static function doUpdate($values, PropelPDO $con = null)
	{
		...
		} else { // $values is moderationFlag object
	...
	/**
	 * Method to DELETE all rows from the moderation_flag table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	public static function doDeleteAll($con = null)
	{
		...
		try {
			...
			moderationFlagPeer::clearRelatedInstancePool();
			$con->commit();
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
	...
	/**
	 * Method perform a DELETE on the database, given a moderationFlag or Criteria object OR a primary key value.
	 *
	 * @param      mixed $values Criteria or moderationFlag object or primary key or array of primary keys
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		if ($values instanceof Criteria) {
		...
		} elseif ($values instanceof moderationFlag) { // it's a model object
			// invalidate the cache for this single object
			moderationFlagPeer::removeInstanceFromPool($values);
			...
			foreach ((array) $values as $singleval) {
				moderationFlagPeer::removeInstanceFromPool($singleval);
			}
		}
	...
	/**
	 * Validates all modified columns of given moderationFlag object.
	 * If parameter $columns is either a single column name or an array of column names
	 * than only those columns are validated.
	 *
	 * NOTICE: This does not apply to primary or foreign keys for now.
	 *
	 * @param      moderationFlag $obj The object to validate.
	...
	public static function doValidate(moderationFlag $obj, $cols = null)
	{
		...
		return BasePeer::doValidate(moderationFlagPeer::DATABASE_NAME, moderationFlagPeer::TABLE_NAME, $columns);
	}
	...
	public static function retrieveByPK($pk, PropelPDO $con = null)
	{
		...
		if (null !== ($obj = moderationFlagPeer::getInstanceFromPool((string) $pk))) {
			return $obj;
		}
...
} // BasemoderationFlagPeer

// This is the static code needed to register the TableMap for this table with the main Propel class.
//
BasemoderationFlagPeer::buildTableMap();
