...
/**
 * Base class that represents a row from the 'permission_to_permission_item' table.
 *
 * 
 *
 * @package Core
...
abstract class BasePermissionToPermissionItem extends BaseObject  implements Persistent {
	 ...
	 * @var        PermissionToPermissionItemPeer
	 */
	...
	/**
	 * Get the [permission_item_id] column value.
	 * 
	 * @return     int
	 */
	public function getPermissionItemId()
	{
	...
	{
		if(!isset($this->oldColumnsValues[PermissionToPermissionItemPeer::ID]))
			$this->oldColumnsValues[PermissionToPermissionItemPeer::ID] = $this->id;
	 ...
	 * @return     PermissionToPermissionItem The current object (for fluent API support)
	 */
	public function setPermissionItemId($v)
	{
		if(!isset($this->oldColumnsValues[PermissionToPermissionItemPeer::PERMISSION_ITEM_ID]))
			$this->oldColumnsValues[PermissionToPermissionItemPeer::PERMISSION_ITEM_ID] = $this->permission_item_id;

		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->permission_item_id !== $v) {
			$this->permission_item_id = $v;
			$this->modifiedColumns[] = PermissionToPermissionItemPeer::PERMISSION_ITEM_ID;
		}

		if ($this->aPermissionItem !== null && $this->aPermissionItem->getId() !== $v) {
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = PermissionToPermissionItemPeer::CREATED_AT;
			}
	...
	public function setUpdatedAt($v)
	{
		...
		if ( $this->updated_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = PermissionToPermissionItemPeer::UPDATED_AT;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 5; // 5 = PermissionToPermissionItemPeer::NUM_COLUMNS - PermissionToPermissionItemPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating PermissionToPermissionItem object", $e);
		}
	}
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				PermissionToPermissionItemPeer::doDelete($this, $con);
				$this->postDelete($con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				PermissionToPermissionItemPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedRows += PermissionToPermissionItemPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if ($this->aPermission !== null) {
				if (!$this->aPermission->validate($columns)) {
					$failureMap = array_merge($failureMap, $this->aPermission->getValidationFailures());
				}
			}

			if ($this->aPermissionItem !== null) {
				if (!$this->aPermissionItem->validate($columns)) {
					$failureMap = array_merge($failureMap, $this->aPermissionItem->getValidationFailures());
				}
			}


			if (($retval = PermissionToPermissionItemPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = PermissionToPermissionItemPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of PermissionToPermissionItem (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     PermissionToPermissionItem Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @var     PermissionToPermissionItem Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      PermissionToPermissionItem $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(PermissionToPermissionItem $copiedFrom)
	{
	 ...
	 *
	 * @return     PermissionToPermissionItemPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new PermissionToPermissionItemPeer();
		}
		return self::$peer;
	}
	...
	public function getPermission(PropelPDO $con = null)
	{
		if ($this->aPermission === null && ($this->permission_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aPermission->addPermissionToPermissionItems($this);
			 */
		}
