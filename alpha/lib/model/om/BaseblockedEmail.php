...
/**
 * Base class that represents a row from the 'blocked_email' table.
 *
 * 
 *
 * @package Core
...
abstract class BaseblockedEmail extends BaseObject  implements Persistent {
	 ...
	 * @return     blockedEmail The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 1; // 1 = blockedEmailPeer::NUM_COLUMNS - blockedEmailPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				blockedEmailPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of blockedEmail (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     blockedEmail Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @var     blockedEmail Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      blockedEmail $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(blockedEmail $copiedFrom)
	{
