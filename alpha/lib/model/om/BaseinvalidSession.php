...
/**
 * Base class that represents a row from the 'invalid_session' table.
 *
 * 
 *
 * @package Core
 * @subpackage model.om
 */
abstract class BaseinvalidSession extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the actions_limit field.
	 * @var        int
	 */
	...
	/**
	 * Get the [actions_limit] column value.
	 * 
	 * @return     int
	 */
	...
	{
		if(!isset($this->oldColumnsValues[invalidSessionPeer::ID]))
			$this->oldColumnsValues[invalidSessionPeer::ID] = $this->id;
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = invalidSessionPeer::ID;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = invalidSessionPeer::CREATED_AT;
			}
		} // if either are not null
	...
	/**
	 * Set the value of [actions_limit] column.
	 * 
	 * @param      int $v new value
	 * @return     invalidSession The current object (for fluent API support)
	 */
	public function setActionsLimit($v)
	{
		if(!isset($this->oldColumnsValues[invalidSessionPeer::ACTIONS_LIMIT]))
			$this->oldColumnsValues[invalidSessionPeer::ACTIONS_LIMIT] = $this->actions_limit;

		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->actions_limit !== $v) {
			$this->actions_limit = $v;
			$this->modifiedColumns[] = invalidSessionPeer::ACTIONS_LIMIT;
		}
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				invalidSessionPeer::doDelete($this, $con);
				$this->postDelete($con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				invalidSessionPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = invalidSessionPeer::doInsert($this, $con);
					...
					$affectedRows += invalidSessionPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = invalidSessionPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = invalidSessionPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(invalidSessionPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of invalidSession (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     invalidSession Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @var     invalidSession Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      invalidSession $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(invalidSession $copiedFrom)
	{
	 ...
	 *
	 * @return     invalidSessionPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new invalidSessionPeer();
		}
		return self::$peer;
	}
