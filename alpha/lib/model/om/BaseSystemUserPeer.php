...
/**
 * Base static class for performing query and update operations on the 'system_user' table.
 *
 * 
 *
 * @package Core
...
abstract class BaseSystemUserPeer {
	...
	/** the column name for the STATUS_UPDATED_AT field */
	...
	/**
	 * An identiy map to hold any loaded instances of SystemUser objects.
	 * This must be public so that other peer classes can access this when hydrating from JOIN
	 * queries.
	 * @var        array SystemUser[]
	 */
	...
	private static $fieldNames = array (
		...
		BasePeer::TYPE_STUDLYPHPNAME => array ('id', 'email', 'firstName', 'lastName', 'sha1Password', 'salt', 'createdBy', 'status', 'isPrimary', 'statusUpdatedAt', 'createdAt', 'updatedAt', 'deletedAt', 'role', ),
		BasePeer::TYPE_COLNAME => array (self::ID, self::EMAIL, self::FIRST_NAME, self::LAST_NAME, self::SHA1_PASSWORD, self::SALT, self::CREATED_BY, self::STATUS, self::IS_PRIMARY, self::STATUS_UPDATED_AT, self::CREATED_AT, self::UPDATED_AT, self::DELETED_AT, self::ROLE, ),
	...
	private static $fieldKeys = array (
		...
		BasePeer::TYPE_STUDLYPHPNAME => array ('id' => 0, 'email' => 1, 'firstName' => 2, 'lastName' => 3, 'sha1Password' => 4, 'salt' => 5, 'createdBy' => 6, 'status' => 7, 'isPrimary' => 8, 'statusUpdatedAt' => 9, 'createdAt' => 10, 'updatedAt' => 11, 'deletedAt' => 12, 'role' => 13, ),
		BasePeer::TYPE_COLNAME => array (self::ID => 0, self::EMAIL => 1, self::FIRST_NAME => 2, self::LAST_NAME => 3, self::SHA1_PASSWORD => 4, self::SALT => 5, self::CREATED_BY => 6, self::STATUS => 7, self::IS_PRIMARY => 8, self::STATUS_UPDATED_AT => 9, self::CREATED_AT => 10, self::UPDATED_AT => 11, self::DELETED_AT => 12, self::ROLE => 13, ),
	 ...
	 * @param      string $column The column name for current table. (i.e. SystemUserPeer::COLUMN_NAME).
	 * @return     string
	 */
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		$criteria->setPrimaryTableName(SystemUserPeer::TABLE_NAME);
		...
		SystemUserPeer::attachCriteriaFilter($criteria);
		...
		$con = SystemUserPeer::alternativeCon ($con, $queryDB);
	...
	public static function updateInstancePool(&$queryResult)
	{
		...
		{
			$objFromPool = SystemUserPeer::getInstanceFromPool($curObject->getPrimaryKey());
			if ($objFromPool === null)
			{
				SystemUserPeer::addInstanceToPool($curObject);
			}
			else
			{
	...
	public static function addInstancesToPool($queryResult)
	{
		...
		{
			SystemUserPeer::addInstanceToPool($curResult);
		}
	}
	...
	{		
		$criteria = SystemUserPeer::prepareCriteriaForSelect($criteria);
		...
		if ($cachedResult !== null)
		{
			SystemUserPeer::filterSelectResults($cachedResult);
		...
		SystemUserPeer::filterSelectResults($queryResult);
	...
	public static function alternativeCon($con, $queryDB = kQueryCache::QUERY_DB_UNDEFINED)
	{
		...
		if($con === null)
			$con = Propel::getConnection(SystemUserPeer::DATABASE_NAME, Propel::CONNECTION_READ);
	...
	{
		$criteria_filter = SystemUserPeer::getCriteriaFilter();
	...
	public static function doCountStmt(Criteria $criteria, PropelPDO $con = null)
	{
		...
		$con = SystemUserPeer::alternativeCon ( $con );
	 ...
	 *
	 * @param      mixed $value A SystemUser object or a primary key value.
	 */
	public static function removeInstanceFromPool($value)
	{
		if (Propel::isInstancePoolingEnabled() && $value !== null) {
			if (is_object($value) && $value instanceof SystemUser) {
				$key = (string) $value->getId();
			} elseif (is_scalar($value)) {
				// assume we've been passed a primary key
				$key = (string) $value;
			} else {
				$e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or SystemUser object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value,true)));
				throw $e;
			}
	 ...
	 * @return     SystemUser Found object or NULL if 1) no instance exists for specified key or 2) instance pooling has been disabled.
	...
	/**
	 * Method to invalidate the instance pool of all tables related to system_user
	...
	public static function populateObjects(PDOStatement $stmt)
	{
		...
		$cls = SystemUserPeer::getOMClass(false);
		// populate the object(s)
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$key = SystemUserPeer::getPrimaryKeyHashFromRow($row, 0);
			if (null !== ($obj = SystemUserPeer::getInstanceFromPool($key))) {
	...
	{
	  $dbMap = Propel::getDatabaseMap(BaseSystemUserPeer::DATABASE_NAME);
	  if (!$dbMap->hasTable(BaseSystemUserPeer::TABLE_NAME))
	  {
	    $dbMap->addTableObject(new SystemUserTableMap());
	  }
	}
	...
	{
		return $withPrefix ? SystemUserPeer::CLASS_DEFAULT : SystemUserPeer::OM_CLASS;
	}

	/**
	 * Method perform an INSERT on the database, given a SystemUser or Criteria object.
	 *
	 * @param      mixed $values Criteria or SystemUser object containing data that is used to create the INSERT statement.
	...
	{
		if ($con === null) {
			$con = Propel::getConnection(SystemUserPeer::DATABASE_NAME, Propel::CONNECTION_WRITE);
		}

		if ($values instanceof Criteria) {
			$criteria = clone $values; // rename for clarity
		} else {
			$criteria = $values->buildCriteria(); // build Criteria from SystemUser object
		}

		if ($criteria->containsKey(SystemUserPeer::ID) && $criteria->keyContainsValue(SystemUserPeer::ID) ) {
			throw new PropelException('Cannot insert a value for auto-increment primary key ('.SystemUserPeer::ID.')');
		}
	...
	/**
	 * Method perform an UPDATE on the database, given a SystemUser or Criteria object.
	 *
	 * @param      mixed $values Criteria or SystemUser object containing data that is used to create the UPDATE statement.
	...
	public static function doUpdate($values, PropelPDO $con = null)
	{
		...
		} else { // $values is SystemUser object
	...
	/**
	 * Method to DELETE all rows from the system_user table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	public static function doDeleteAll($con = null)
	{
		...
		try {
			...
			SystemUserPeer::clearRelatedInstancePool();
			$con->commit();
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
	...
	/**
	 * Method perform a DELETE on the database, given a SystemUser or Criteria object OR a primary key value.
	 *
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		if ($values instanceof Criteria) {
		...
		} elseif ($values instanceof SystemUser) { // it's a model object
			// invalidate the cache for this single object
			SystemUserPeer::removeInstanceFromPool($values);
			...
			foreach ((array) $values as $singleval) {
				SystemUserPeer::removeInstanceFromPool($singleval);
			}
		}
	...
	/**
	 * Validates all modified columns of given SystemUser object.
	 * If parameter $columns is either a single column name or an array of column names
	 * than only those columns are validated.
	 *
	 * NOTICE: This does not apply to primary or foreign keys for now.
	 *
	 * @param      SystemUser $obj The object to validate.
	...
	public static function doValidate(SystemUser $obj, $cols = null)
	{
		...
		return BasePeer::doValidate(SystemUserPeer::DATABASE_NAME, SystemUserPeer::TABLE_NAME, $columns);
	}
	...
	public static function retrieveByPK($pk, PropelPDO $con = null)
	{
		...
		if (null !== ($obj = SystemUserPeer::getInstanceFromPool((string) $pk))) {
			return $obj;
		}
...
//
BaseSystemUserPeer::buildTableMap();
