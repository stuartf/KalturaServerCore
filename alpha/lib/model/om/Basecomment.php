...
abstract class Basecomment extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the base_date field.
	 * @var        string
	 */
	protected $base_date;
	...
	/**
	 * Get the [optionally formatted] temporal [base_date] column value.
	 * 
	 ...
	 */
	public function getBaseDate($format = '%x')
	{
		if ($this->base_date === null) {
			return null;
		}


		if ($this->base_date === '0000-00-00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->base_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->base_date, true), $x);
			}
		}
	...
	/**
	 * Sets the value of [base_date] column to a normalized version of the date/time value specified.
	 * 
	...
	public function setBaseDate($v)
	{
		...
		if ( $this->base_date !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->base_date !== null && $tmpDt = new DateTime($this->base_date)) ? $tmpDt->format('Y-m-d') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->base_date = ($dt ? $dt->format('Y-m-d') : null);
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->reply_to = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
	 ...
	 *
	 * @var     comment Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      comment $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(comment $copiedFrom)
	{
