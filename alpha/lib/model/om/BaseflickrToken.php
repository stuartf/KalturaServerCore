...
/**
 * Base class that represents a row from the 'flickr_token' table.
 *
 * 
 *
 * @package Core
...
abstract class BaseflickrToken extends BaseObject  implements Persistent {
	...
	/**
	 * Initializes internal state of BaseflickrToken object.
	 * @see        applyDefaults()
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->response = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->is_valid = ($row[$startcol + 5] !== null) ? (boolean) $row[$startcol + 5] : null;
			...
			return $startcol + 8; // 8 = flickrTokenPeer::NUM_COLUMNS - flickrTokenPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				flickrTokenPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of flickrToken (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     flickrToken Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @var     flickrToken Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      flickrToken $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(flickrToken $copiedFrom)
	{
