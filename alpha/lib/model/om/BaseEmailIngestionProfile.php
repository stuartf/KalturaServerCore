...
/**
 * Base class that represents a row from the 'email_ingestion_profile' table.
 *
 * 
 *
 * @package Core
...
abstract class BaseEmailIngestionProfile extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the mailbox_id field.
	 * @var        string
	 */
	...
	/**
	 * Initializes internal state of BaseEmailIngestionProfile object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [mailbox_id] column value.
	 * 
	 * @return     string
	 */
	 ...
	 * @return     EmailIngestionProfile The current object (for fluent API support)
	 */
	...
	/**
	 * Set the value of [mailbox_id] column.
	 * 
	 * @param      string $v new value
	 * @return     EmailIngestionProfile The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->mailbox_id = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->partner_id = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
			$this->conversion_profile_2_id = ($row[$startcol + 6] !== null) ? (int) $row[$startcol + 6] : null;
			$this->moderation_status = ($row[$startcol + 7] !== null) ? (int) $row[$startcol + 7] : null;
			...
			return $startcol + 12; // 12 = EmailIngestionProfilePeer::NUM_COLUMNS - EmailIngestionProfilePeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				EmailIngestionProfilePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of EmailIngestionProfile (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     EmailIngestionProfile Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @var     EmailIngestionProfile Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      EmailIngestionProfile $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(EmailIngestionProfile $copiedFrom)
	{
	 ...
	 *
	 * @return     EmailIngestionProfilePeer
	 */
