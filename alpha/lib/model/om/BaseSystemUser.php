...
/**
 * Base class that represents a row from the 'system_user' table.
 *
 * 
 *
 * @package Core
...
abstract class BaseSystemUser extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the status_updated_at field.
	 * @var        string
	 */
	...
	/**
	 * Initializes internal state of BaseSystemUser object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [status_updated_at] column value.
	 * 
	...
	public function getStatusUpdatedAt($format = 'Y-m-d H:i:s')
	{
		...
		if ($this->status_updated_at === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->status_updated_at);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->status_updated_at, true), $x);
			}
		}
	 ...
	 * @return     SystemUser The current object (for fluent API support)
	 */
	...
	/**
	 * Sets the value of [status_updated_at] column to a normalized version of the date/time value specified.
	 * 
	...
	public function setStatusUpdatedAt($v)
	{
		...
		if ( $this->status_updated_at !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->status_updated_at !== null && $tmpDt = new DateTime($this->status_updated_at)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->status_updated_at = ($dt ? $dt->format('Y-m-d H:i:s') : null);
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->is_primary = ($row[$startcol + 8] !== null) ? (boolean) $row[$startcol + 8] : null;
			$this->status_updated_at = ($row[$startcol + 9] !== null) ? (string) $row[$startcol + 9] : null;
			$this->created_at = ($row[$startcol + 10] !== null) ? (string) $row[$startcol + 10] : null;
			$this->updated_at = ($row[$startcol + 11] !== null) ? (string) $row[$startcol + 11] : null;
			$this->deleted_at = ($row[$startcol + 12] !== null) ? (string) $row[$startcol + 12] : null;
			$this->role = ($row[$startcol + 13] !== null) ? (string) $row[$startcol + 13] : null;
			$this->resetModified();
			...
			return $startcol + 14; // 14 = SystemUserPeer::NUM_COLUMNS - SystemUserPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = SystemUserPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				SystemUserPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of SystemUser (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     SystemUser Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @var     SystemUser Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      SystemUser $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(SystemUser $copiedFrom)
	{
