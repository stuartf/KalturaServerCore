...
class KalturaPartnerService extends KalturaServiceBase
{
	...
	function getWidgetStatisticsAction($startDate, $endDate)
	{
		$kparams = array();
		$this->client->addParam($kparams, "start_date", $startDate);
		$this->client->addParam($kparams, "end_date", $endDate);
		$this->client->queueServiceActionCall("partner", "getWidgetStatisticsAction", $kparams);
		if ($this->client->isMultiRequest())
			return null;
		$resultObject = $this->client->doQueue();
		$this->client->throwExceptionIfError($resultObject);
		//$this->client->validateObjectType($resultObject, "KalturaPartnerWidgetsStatistics");
		return $resultObject;
	}
}
